...
uint8_t Serial_RxPakage[4];  //存放接收的数据包
uint8_t Serial_TxPakage[4];  //存放发送的数据包
uint8_t Serial_TxState;		 
...
void Serial_SendPakage(void)
{
	...
	Serial_SendArray(Serial_TxPakage,4);
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE)==SET){
		...
		if(RxDATA == 0XFF && Serial_TxState==0)
		{
		...
		else if(Serial_TxState==1)
		{
			...
			{	
				Serial_RxCounter = 0;  //如果收完4个载荷 值0 改变状态
				Serial_TxState = 2;	   //切换成转换包尾的状态
			}
		}
