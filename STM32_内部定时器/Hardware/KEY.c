...
uint8_t KEY_GetNum(void)  //返回值是8位Char 0-255
{
	...
	if(GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_1)==0)  //读取外部引脚的按下值 0代表按下
	{
			...
			KeyNum = 1;   //赋值1 代表按键1按下
	}	
	/*按键1*/
	
	/*判断按键2*/
		if(GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_11)==0)  //读取外部引脚的按下值 0代表按下
	{
			Delay_ms(20); //消按键按下的抖动
			while(GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_11)==0); //如果按键一直没有松开则卡在这
			Delay_ms(20); //消按下松开的抖动
			KeyNum = 2;   //赋值2 代表按键2按下
	}	
	/*按键2*/
	
	
	return KeyNum; 	//返回值代表哪个按键生效
}
