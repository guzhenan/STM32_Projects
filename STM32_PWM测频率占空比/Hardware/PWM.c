...
/****************************输出比较器初始化****************************/	
	...
	TIM_OC1Init(TIM2, &TIM_OCInitStruct);                               //设置通道CH
/****************************输出比较器初始化****************************/	
	TIM_Cmd(TIM2, ENABLE);									 //时钟生效 PWM的波形就能通过PA0输出了
}

void PWM_SetCompare1(uint16_t compare)  //在运行过程中设置CCR
{
	TIM_SetCompare1(TIM2, compare);
}


void PWM_SetPrescaler(uint16_t x)  //设置预分频值
{
	TIM_PrescalerConfig(TIM2, x, TIM_PSCReloadMode_Immediate);
}
